...
/// Toggle the protocol-wide unstaking pause. Updates the Cadence flag AND mirrors the change
/// to the Solidity LSPVault via `EVMRoute.setUnstakingPaused` so the two sides cannot drift.
transaction(paused: Bool) {
    prepare(signer: auth(BorrowValue) &Account) {
        ...
        admin.setUnstakingPaused(paused: paused)
    }
}
