...
///
/// Mirrors the non-EVM surface of the real contract, including the IncrementFi-
/// style **delegator manager** (`DelegatorSet`): multi-slot NodeDelegator
/// custody, pending-claim accounting, and `access(account)` FLOW primitives
/// used by `LiquidStaking` stake / unstake / withdraw.
access(all) contract LiquidStakingConfig {
    ...
    access(all) resource DelegatorSet {
        ...
        /// Reserve FLOW for a redemption. Prefers unallocated exiting buckets
        /// then opens a new `requestUnstaking` on Active slots only.
    ...
    // ---- access(account) manager surface for LiquidStaking ----
